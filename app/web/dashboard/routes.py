...
def show_dashboard(*args, permission_level, connection,**kwargs):
    ...
    try:
        cur.execute(
            sql.SQL("SELECT A.uuid, A.title, A.publish_date, B.display_name FROM sloth_posts AS A INNER JOIN sloth_post_types AS B ON B.uuid = A.post_type WHERE post_status = %s LIMIT 10"), ['published']
        )
    ...
    return render_template("dashboard.html", post_types=postTypesResult, permission_level=permission_level, messages=messages, recent_posts=recent_posts, drafts=drafts, upcoming_posts=upcoming_posts)