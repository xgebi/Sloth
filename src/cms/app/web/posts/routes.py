...
@posts.route("/posts/<post_type>/<post_id>")
@authorize_web(0)
@db_connection
def show_post_edit(connection, post_type, post_id):
	if connection is None:
		return render_toe(template="settings.toe", path_to_templates=current_app.config["TEMPLATES_PATH"], data={ "error": "No connection to database" })
	post_type_info = {
		"uuid": post_type
	...
	return render_toe(template="post-edit.toe", path_to_templates=current_app.config["TEMPLATES_PATH"], data={ "posts_list":items, "post_types": postTypesResult, "post_type":post_type_info } )

@posts.route("/posts/<post_type>/<post_id>/delete")
@authorize_web(0)
@db_connection
def delete_post(connection, post_type, post_id):
