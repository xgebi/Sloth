...
pub(crate) fn create_token(user_id: String) -> Option<(String, u128)> {
    let mut db_result = connect();
    if let Ok(mut db) = db_result {
...
pub(crate) fn refresh_token(user_id: String, token: String) -> bool {
    let mut db_result = connect();
    if let Ok(mut db) = db_result {
        if !is_token_valid(user_id, token, db) {
            return false;
        }
        let _stmt = include_str!("queries/create_token.sql");
        let expiration = SystemTime::now()
            ...
            .expect("Time went backwards")
            .as_millis() + (30 * 60 * 1000);
        let result = db.execute(_stmt, &[&expiration, &user_id, &token]);
        return result.is_ok();
    }
    false
}

pub(crate) fn is_token_valid(user_id: String, token: String, mut client: Client) -> bool {
